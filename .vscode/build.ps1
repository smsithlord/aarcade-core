...
# Build command
$buildCmd = @"
"$vsPath" x64 && cl.exe /Zi /EHsc /nologo /std:c++17 /I"$root\include" /I"$root\aarcade_core" /I"$vcpkgInclude" /Fe"$root\x64\Release\aarcade-core.exe" "$root\aarcade-core.cpp" "$root\aarcade_core\MainApp.cpp" "$root\aarcade_core\JSBridge.cpp" "$root\aarcade_core\ImageLoader.cpp" "$root\aarcade_core\ConsoleLogger.cpp" /link /MACHINE:X64 /LIBPATH:"$root\lib" /LIBPATH:"$vcpkgLib" AppCore.lib Ultralight.lib UltralightCore.lib WebCore.lib sqlite3.lib libcurl.lib
"@
