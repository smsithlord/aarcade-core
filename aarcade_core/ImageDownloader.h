...
// Result structure for image downloading
// NOTE: This class manages background HTTP downloads using IImageDownloader implementations
// NOT currently connected or used. Use the new ImageLoader class for Ultralight-based rendering.
...
private:
    ...
    struct DownloadJob {
        std::string url;
        std::function<void(const ImageDownloadResult&)> callback;
    };
    ...
    void debugOutput(const std::string& message) {
        std::string debugMsg = "[ImageDownloader] " + message;
    ...
    void processDownload(const std::string& url, std::function<void(const ImageDownloadResult&)> callback) {
...
public:
    ImageDownloader(std::unique_ptr<IImageDownloader> downloader)
        ...
        debugOutput("ImageDownloader initialized. Cache path: " + fullPath);

        // Start worker thread
        workerThread_ = std::thread(&ImageDownloader::workerThreadFunc, this);
    }
    ...
    void downloadImage(const std::string& url, std::function<void(const ImageDownloadResult&)> callback) {
        // Add job to queue
        {
            std::lock_guard<std::mutex> lock(queueMutex_);
